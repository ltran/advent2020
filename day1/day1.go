...
func SumTwo(filepath string, sum int) int {
	entries := adventutil.ToIntSlice(adventutil.ReadFile(filepath))

	terms, ok := DoesSumTwo(entries, sum)
	if !ok {
		fmt.Println("Did not find any terms that would sum up to", sum)
	}

	return terms[0] * terms[1]
}

func SumThree(filepath string, sum int) int {
	entries := adventutil.ToIntSlice(adventutil.ReadFile(filepath))

	terms, ok := DoesSumThree(entries, sum)
	if !ok {
		fmt.Println("Did not find any terms that would sum up to", sum)
	}
...
func DoesSumTwo(entries []int, sum int) ([2]int, bool) {
	dict := map[int]struct{}{}
	for _, entry := range entries {
		_, ok := dict[sum-entry]
		if ok {
			return [2]int{(sum - entry), entry}, true
		}
		dict[entry] = struct{}{}
	}
...
func DoesSumThree(entries []int, sum int) ([3]int, bool) {
	for i := 0; i < len(entries); i++ {
		tmpEntries := entries[i+1 : len(entries)-1]
		terms, ok := DoesSumTwo(tmpEntries, sum-entries[i])
		if ok {
			ans := [3]int{entries[i], terms[0], terms[1]}
			return ans, true
		}
	}
